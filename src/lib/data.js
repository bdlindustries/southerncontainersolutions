export const SEED_INVENTORY = [
  {
    ...
    features: [
      ...
      "36\" Man Door with Deadbolt + Operational Cargo Doors",
    ],
    listingFeatures: [
  ...
  },
  {
    ...
    features: [
      ...
      "36\" Steel Man Door with Deadbolt + Operational Cargo Doors",
    ],
    listingFeatures: [
  ...
  },
  {
    ...
    stripeLink: "https://buy.stripe.com/test_placeholder",
    description:
      "40' one trip unit with a full 20' office build (same spec as our 20' office) plus a dedicated 20' storage bay. Includes a partition wall with interior man door, and storage lighting and power.",
    listingDescription:
    ...
    features: [
      ...
      "42\" Office Window, Man Door with Deadbolt, Cargo Doors",
    ],
    listingFeatures: [
      ...
      `Windows (office)\nOne 42" x 30" sliding window with steel security bars.`,
      `Exterior & cargo access\nOne 36" man door with deadbolt. Fully operational cargo doors for loading the storage bay and oversized items.`,
...
export function enrichInventoryFromSeed(item) {
  ...
  return {
    ...
    listingFeatures,
    features: Array.isArray(item.features) && item.features.length > 0 ? item.features : seed.features,
    deliveryBlurb: item.deliveryBlurb || seed.deliveryBlurb,
    customizationBlurb: item.customizationBlurb || seed.customizationBlurb,
    stripeLink: item.stripeLink || seed.stripeLink,
    priceStr: item.priceStr || seed.priceStr,
  };
}

export function coerceImageList(raw) {
  if (raw == null) return [];
...
export function normalizeInventoryItemFromFirestore(id, data) {
